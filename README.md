...
<h1 align="center">MyFinans (v12.4.3)</h1>

<p align="center">
  <a href="https://github.com/eekilinc/MyFinans/releases/latest"><img alt="Son Sürüm" src="https://img.shields.io/badge/sürüm-v12.4.3--final-8b5cf6?style=flat-square&logo=android"></a>
...
<p align="center">
  <a href="https://github.com/eekilinc/MyFinans/releases/latest"><strong>↓ Final APK'yı İndir (v12.4.3)</strong></a>
  · <a href="#-ekran-görüntüleri">Ekran Görüntüleri</a>
  · <a href="#-özellikler-ve-çalışma-mantığı">Özellikler</a>
  · <a href="#-v1243-ile-gelen-yenilikler">v12.4.3 Yenilikleri</a>
...
## 🚀 v12.4.3 ile Gelen Yenilikler

- 📊 **İstatistik & Trend Ekranı Açık Tema (Light Mode) Uyumu**:
  - Daha önce koyu renk sınıflarıyla kısıtlı olan İstatistik & Trend ekranı baştan aşağı Açık Tema uyumlu hale getirildi.
  - Yıllık harcama, ödenen ve kalan tutar kartları açık temada ferah beyaz zemin, yüksek kontrastlı renkli üst aksanlar ve yumuşak gölgeler kazandı.
  - Aylık karşılaştırma grafik çubukları, ay başlıkları ve 12 aylık detaylı özet tablosu hem açık hem koyu temada kusursuz okunabilirliğe ulaştırıldı.
  - Dinamik yıl açıklaması (`history_trends_subtitle`), kayıtlı ay sayaçları (`months_recorded`) ve boş durum göstergesi eklendi.
- 🏢 **Firmalar Ekranında Sıfır Taşma (Mobile Responsive Overflow Fix)**:
  - Hızlı firma ekleme formu dar mobil ekranlarda (`flex flex-col sm:flex-row`, `min-w-0`, `w-full`) buton ve metin kutusunun ekran dışına taşmasını önleyecek şekilde esnekleştirildi.
  - Firma kartlarındaki uzun firma adları otomatik kırpma (`truncate`) ile güvenli alana alındı; inline düzenleme ve `Düzenle`/`Sil` butonlarının dar ekranlarda kaybolması veya taşması tamamen engellendi.
  - Firma detay modalının başlık ve kapatma butonları responsive güvenliğe alındı.
...
## 🚀 v12.4.0 ile Gelen Yenilikler

- 👤 **Geliştirici Bilgisi Güncellendi**: "Hakkında" penceresinde ve dökümantasyonda geliştirici ismi **Ekrem Eşref KILINÇ (@eekilinc)** olarak tanımlandı.
- 🎨 **Fresh & Minimalist Yeni İkon Geometrisi**:
  - Karmaşık eski madeni para simgesi kaldırılarak, modern fintech tasarım standartlarında (Revolut/Wise/Apple Wallet standardı) minimalist, ferah ve güçlü bir logo üretildi.
  - Gece mavisi/indigo zemin üzerinde, akıcı ve pürüzsüz kıvrımlı beyaz "M" monogramı ve neon camgöbeği (cyan) büyüme kıvılcımı yerleştirildi.
  - Android launcher adaptive icon (tüm mipmap yoğunluklarında safe-zone uyumlu), yuvarlak simge ve açılış (splash) ekranları yenilendi.
- 🌈 **Canlı ve Dinamik Renk Paleti Entegrasyonu**:
  - 5 tema paleti (Asil Mor, Zümrüt Yeşil, Okyanus Mavi, Gün Batımı Gül, Titanyum Gri) Tailwind CSS v4 renk motoruna dinamik olarak bağlandı.
  - Ayarlardan renk paleti seçildiğinde uygulamanın tüm butonları, grafikleri, rozetleri ve aksanları anında seçilen renge bürünüyor.
- 🔐 **PIN ve Biyometrik Kilit Senkronizasyonu & Çeviri Düzeltmesi**:
  - `change_pin`, `setup_pin`, `disable_pin`, `security_lock` çevirileri eksiksiz tanımlandı ve `PinLock` motoruyla %100 senkronize edildi.
