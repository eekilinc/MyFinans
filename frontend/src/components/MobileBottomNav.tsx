...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-40 sm:hidden bg-white/95 dark:bg-slate-950/95 backdrop-blur-2xl border-t border-slate-200 dark:border-slate-800/90 shadow-[0_-8px_25px_rgba(0,0,0,0.08)] dark:shadow-[0_-8px_25px_rgba(0,0,0,0.5)] pb-safe transition-colors">
      <div className="flex items-center justify-around h-16 px-2">
        ...
        <button
          ...
          className="flex flex-col items-center justify-center w-16 py-1 transition-all active:scale-95 text-slate-600 dark:text-slate-300 hover:text-purple-600 dark:hover:text-white font-medium"
          aria-label={t('settings') || 'Ayarlar'}
        >
          <div className="p-1 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/80 text-slate-600 dark:text-slate-300 hover:text-purple-600 dark:hover:text-purple-300 transition-colors">
            <Settings className="w-5 h-5" />
          </div>
          <span className="text-[10px] mt-0.5 tracking-tight text-slate-600 dark:text-slate-300 truncate">
            {t('settings') || 'Ayarlar'}
          </span>
        </button>
      </div>
