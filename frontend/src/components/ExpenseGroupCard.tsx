...
export const ExpenseGroupCard: React.FC<ExpenseGroupCardProps> = ({
  ...
  return (
    <div className="rounded-3xl bg-slate-900/80 border border-slate-800/90 shadow-xl overflow-hidden transition-all duration-200">
      {/* Group Card Header */}
      <div className="p-4 sm:p-5 flex flex-col gap-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1 text-xs text-slate-400 flex-wrap">
                {group.bank_name && (
                  <span className="font-semibold text-slate-300">{group.bank_name}</span>
                )}
                {group.statement_day ? (
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="p-2 rounded-xl bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 border border-purple-500/30 transition-all active:scale-95"
            ...
            <button
              ...
              className="p-2 rounded-xl bg-slate-800 hover:bg-rose-950/40 text-slate-400 hover:text-rose-400 transition-all active:scale-95"
              title={t('delete')}
            >
        ...
        <div className="pt-3 border-t border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-4 flex-wrap">
            <div>
              <span className="text-[11px] text-slate-400 font-medium block">Aylık Tutar</span>
              <span className="text-base font-black text-white">
                {formatCurrency(totalAmount)}
              </span>
            </div>
            <div>
              <span className="text-[11px] text-slate-400 font-medium block">Kalan</span>
              <span
                className={`text-base font-black ${
                  unpaidAmount > 0 ? 'text-rose-400' : 'text-emerald-400'
                }`}
              >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700/50 text-xs font-bold text-slate-200 transition-colors"
            >
              <span>
      ...
      {isExpanded && (
        <div className="p-4 sm:p-5 bg-slate-950/40 border-t border-slate-800/80 space-y-2.5">
